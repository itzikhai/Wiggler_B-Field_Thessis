...
def read_rand_files():
    rand0 = np.empty([160,2],float)
    rand0 = np.zeros([160,2],float)
    ...
    fin = open('rand_0_1_dev.txt', 'r')
    rand0_str = fin.read()
    rand0_list = rand0_str.split('\n')
    for line in range(160):
        rand0[line,0] = float(rand0_list[line].split(',')[0])
        rand0[line,1] = float(rand0_list[line].split(',')[1])
    fin.close()
...
rand0, rand1, rand2 = read_rand_files()
rand = np.zeros([160,2], float)
# selecting the 10% (rand0) or 5% (rand1) or 2.5% (rand2) deviation:
rand[:,0] = rand1[:,0]
rand[:,1] = rand1[:,1] 
